...
import { createbookingTool, getAvailabilityTool } from "./booking-cal.js";
import { getPisos2 } from "./pdf-loader_tool.js";
...
import { INMUEBLE_PROPS } from "./products_finder/schemas.js";
import { productsFinder } from "./products_finder/tools.js";
import { obtener_info_usuario } from "./utils/tools.js";
import { contextPrompt } from "./utils/context.mjs";
...
const tools = [productsFinder, obtener_info_usuario];
...
const toolNodo = async (
  ...
  if (lastMessage?.tool_calls?.length) {
    ...
    if (toolName === "products_finder") {
      const response = await productsFinder.invoke({
        prompt: toolArgs.query,
        props: INMUEBLE_PROPS,
      });

      if (response) {
        toolMessage = response.message as ToolMessage;
      } else {
        toolMessage = new ToolMessage(
          "Ha ocurrido algun error al buscar las propiedades, por favor intentalo nuevamente.",
          tool_call_id,
          "products_finder"
        );
      }
